...
public class PlantDBMapper {
    ...
    public static Plant mapToPOJO(PlantRow plantRow) {
        return Plant.builder().name(plantRow.getName()).scientificName(plantRow.getScientificName()).minimumTolerance(Temperature.fromKelvin(plantRow.getColdThresholdK())).uuid(UUID.fromString(plantRow.getUuid())).build();
    }
}
