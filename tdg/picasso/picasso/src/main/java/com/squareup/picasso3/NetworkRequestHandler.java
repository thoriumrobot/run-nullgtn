...
final class NetworkRequestHandler extends RequestHandler {
  ...
  final @Nullable Callback callback) {
    ...
    callFactory.newCall(callRequest).enqueue(new okhttp3.Callback() {
      @Override public void onResponse(@Nullable Call call, Response response) {
  ...
  @Override @Nullable int getRetryCount() {
    return 2;
  }
  ...
  @Override @Nullable boolean supportsReplay() {
    return true;
  }

  @Nullable private static okhttp3.Request createRequest(Request request) {
