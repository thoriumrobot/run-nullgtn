...
public class HTTPForecastServiceOWMImpl implements HTTPForecastService {
    ...
    public Observable<WeatherData> getForecast(SimpleWeatherLocation location) {
        return owmService.getOWMForecast(location.getLat(), location.getLon(), OPEN_WEATHER_MAP_API_KEY).subscribeOn(Schedulers.io()).doOnError(Throwable::printStackTrace).filter(Response::isSuccessful).map(Response::body).flatMap(owmForecast -> {
    ...
    private WeatherData mapToPOJO(OWMForecast owmForecast, SimpleWeatherLocation simpleWeatherLocation) {
        @Nullable final List<ForecastHour> forecastHourList = Optional.ofNullable(owmForecast.list).orElse(Collections.emptyList()).stream().flatMap(period -> {
            if (period.dt == null || period.forecast == null || period.forecast.temp == null || period.wind == null || period.wind.speed == null)
                return Stream.empty();
            else
                return Stream.of(ForecastHour.builder().hour(Instant.ofEpochSecond(period.dt)).windspeed(Windspeed.fromMetersPerSecond(period.wind.speed)).uuid(UUID.randomUUID()).temperature(Temperature.fromKelvin(period.forecast.temp)).build());
        }).collect(Collectors.toList());
        String name = "";
        if (owmForecast.city != null && owmForecast.city.name != null)
            name = owmForecast.city.name;
        return WeatherData.builder().forecastHours(forecastHourList).syncInstant(Instant.now()).weatherLocation(WeatherLocation.builder().setPlaceString(name).setLat(simpleWeatherLocation.getLat()).setLon(simpleWeatherLocation.getLon()).build()).build();
    }
}
