...
final class UnboundedLocalCache<K, V> implements LocalCache<K, V> {
    ...
    @Override
    public boolean @Nullable hasWriteTime() {
        return false;
    }

    /* ---------------- Cache -------------- */
    @Override
    @Nullable public V getIfPresent(Object key, boolean recordStats) {
    ...
    @Override
    @Nullable public V getIfPresentQuietly(Object key, long[] writeTime) {
        return data.get(key);
    }
    ...
    @Override
    public Map<K, V> @Nullable getAllPresent(Iterable<?> @Nullable keys) {
    ...
    @Override
    public boolean @Nullable hasRemovalListener() {
        return (removalListener != @Nullable null);
    }

    @Override
    @SuppressWarnings("NullAway")
    public RemovalListener<K, V> @Nullable removalListener() {
        return removalListener;
    }

    @Override
    public void notifyRemoval(@Nullable K key, @Nullable V @Nullable value, RemovalCause cause) {
    ...
    @Override
    public Executor @Nullable executor() {
        return executor;
    }
    ...
    /* ---------------- JDK8+ Map extensions -------------- */
    @Override
    public void forEach(BiConsumer<? super K, ? super V> @Nullable action) {
        data.forEach(action);
    }

    @Override
    public void @Nullable replaceAll(BiFunction<? super K, ? super V, ? extends V> @Nullable function) {
        ...
        if (notificationKey[0] != @Nullable null) {
            notifyRemoval(notificationKey[0], notificationValue[0], RemovalCause.REPLACED);
        }
    }

    @Override
    public V @Nullable computeIfAbsent(K @Nullable key, Function<? super K, ? extends V> @Nullable mappingFunction, boolean recordStats, boolean recordLoad) {
    ...
    @Override
    @Nullable public V computeIfPresent(K key, BiFunction<? super K, ? super V, ? extends V> remappingFunction) {
    ...
    @Override
    public V compute(K key, BiFunction<? super K, ? super V, ? extends V> @Nullable remappingFunction, boolean recordMiss, boolean recordLoad) {
        requireNonNull(remappingFunction);
        return remap(key, statsAware(remappingFunction, recordMiss, recordLoad));
    }

    @Override
    public V merge(K key, V @Nullable value, BiFunction<? super V, ? super V, ? extends V> remappingFunction) {
        requireNonNull(remappingFunction);
     ...
     */
    V remap(K @Nullable key, BiFunction<? super K, ? super V, ? extends V> remappingFunction) {
        // ensures that the removal notification is processed after the removal has completed
        @SuppressWarnings({ "unchecked", "rawtypes" })
        V[] oldValue = (V[]) new Object[1];
        RemovalCause[] cause = new RemovalCause[1];
        V nv = data.compute(key, (K k, V value) -> {
            ...
            cause[0] = (newValue == null) ? RemovalCause.EXPLICIT : RemovalCause.REPLACED;
            if (hasRemovalListener() && (value != @Nullable null) && (newValue != value)) {
                oldValue[0] = value;
            }
            return newValue;
        });
    ...
    @Override
    public boolean @Nullable containsKey(Object @Nullable key) {
        return data.containsKey(key);
    }

    @Override
    public boolean containsValue(Object @Nullable value) {
        return data.containsValue(value);
    }

    @Override
    @Nullable public V get(Object @Nullable key) {
    ...
    @Override
    @Nullable public V put(K key, V value, boolean @Nullable notifyWriter) {
        requireNonNull(value);
        // ensures that the removal notification is processed after the removal has completed
        @SuppressWarnings({ "unchecked", "rawtypes" })
        V[] oldValue = (V[]) new Object[1];
    ...
    @Override
    public void @Nullable putAll(Map<? extends K, ? extends V> map) {
        if (!hasRemovalListener() && (writer == CacheWriter.disabledWriter())) {
            data.putAll(map);
            return;
        }
    ...
    @Override
    public boolean @Nullable remove(Object key, Object value) {
        if (value == null) {
            requireNonNull(key);
            return false;
        }
    ...
    @Override
    public Set<K> @Nullable keySet() {
        final Set<K> ks = keySet;
        return (ks == null) ? (keySet = new KeySetView<>(this)) : ks;
    }

    @Override
    public Collection<V> @Nullable values() {
    ...
    static final class KeyIterator<K> implements Iterator<K> {
        ...
        final UnboundedLocalCache<K, ?> @Nullable cache;
    ...
    static final class ValuesIterator<K, V> implements Iterator<V> {
        ...
        final UnboundedLocalCache<K, V> @Nullable cache;
        ...
        @Override
        public boolean @Nullable hasNext() {
            return iterator.hasNext();
        }
    ...
    static final class EntryIterator<K, V> implements Iterator<Entry<K, V>> {
        ...
        @Nullable Entry<K, V> @Nullable entry;
    ...
    static final class EntrySpliterator<K, V> implements Spliterator<Entry<K, V>> {
        ...
        @Override
        @Nullable public EntrySpliterator<K, V> trySplit() {
            Spliterator<Entry<K, V>> split = spliterator.trySplit();
            return (split == @Nullable null) ? @Nullable null : new EntrySpliterator<>(cache, split);
        }
    ...
    static class UnboundedLocalManualCache<K, V> implements LocalManualCache<K, V>, Serializable {
        ...
        private void readObject(ObjectInputStream @Nullable stream) throws InvalidObjectException {
    ...
    static final class UnboundedPolicy<K, V> implements Policy<K, V> {
        ...
        @Override
        public boolean @Nullable isRecordingStats() {
            return isRecordingStats;
        }

        @Override
        public Optional<Eviction<K, V>> @Nullable eviction() {
            return Optional.empty();
        }

        @Override
        public Optional<Expiration<K, V>> @Nullable expireAfterAccess() {
            return Optional.empty();
        }

        @Override
        public Optional<Expiration<K, V>> @Nullable expireAfterWrite() {
            return Optional.empty();
        }

        @Override
        public Optional<Expiration<K, V>> @Nullable refreshAfterWrite() {
            return Optional.empty();
        }
    }
    ...
    static final class UnboundedLocalLoadingCache<K, V> extends UnboundedLocalManualCache<K, V> implements LocalLoadingCache<K, V> {
        ...
        final CacheLoader<? super K, V> @Nullable loader;
    ...
    static final class UnboundedLocalAsyncCache<K, V> implements LocalAsyncCache<K, V>, Serializable {
        ...
        @Nullable Policy<K, V> @Nullable policy;
        ...
        @Override
        public Cache<K, V> @Nullable synchronous() {
            return (cacheView == null) ? (cacheView = new CacheView<>(this)) : cacheView;
        }

        @Override
        public Policy<K, V> policy() {
            return (policy == @Nullable null) ? (policy = new UnboundedPolicy<>(cache.isRecordingStats)) : policy;
        }

        private void @Nullable readObject(ObjectInputStream stream) throws InvalidObjectException {
            throw new InvalidObjectException("Proxy required");
        }
    ...
    static final class UnboundedLocalAsyncLoadingCache<K, V> extends LocalAsyncLoadingCache<K, V> implements Serializable {
        ...
        private static final long @Nullable serialVersionUID = 1;
        ...
        @Override
        public LocalCache<K, CompletableFuture<V>> @Nullable cache() {
            return cache;
        }
