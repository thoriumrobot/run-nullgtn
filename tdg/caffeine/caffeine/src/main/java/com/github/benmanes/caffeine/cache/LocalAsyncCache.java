...
interface LocalAsyncCache<K, V> extends AsyncCache<K, V> {
    ...
    @Override
    @Nullable default CompletableFuture<V> getIfPresent(@NonNull Object key) {
    ...
    default CompletableFuture<V> get(K key, BiFunction<? super K, Executor, CompletableFuture<V>> mappingFunction, boolean @Nullable recordStats) {
    ...
    abstract class @Nullable AbstractCacheView<K, V> implements Cache<K, V>, Serializable {

        @Nullable transient AsMapView<K, V> asMapView;

        abstract LocalAsyncCache<K, V> @Nullable asyncCache();
    ...
    final class AsMapView<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
        ...
        @Override
        @Nullable public V computeIfAbsent(K key, Function<? super K, ? extends V> mappingFunction) {
            requireNonNull(mappingFunction);
        ...
        @Override
        @Nullable public V compute(K key, BiFunction<? super K, ? super V, ? extends V> remappingFunction) {
            requireNonNull(remappingFunction);
            boolean[] computed = { false };
            for (; ; ) {
                CompletableFuture<V> future = delegate.get(key);
        ...
        @Override
        @Nullable public V merge(K key, V value, BiFunction<? super V, ? super V, ? extends V> remappingFunction) {
            requireNonNull(value);
            requireNonNull(remappingFunction);
