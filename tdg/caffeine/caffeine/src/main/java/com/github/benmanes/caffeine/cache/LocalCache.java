...
interface LocalCache<K, V> extends ConcurrentMap<K, V> {
    ...
    @Override
    @Nullable default V compute(K key, BiFunction<? super K, ? super V, ? extends V> remappingFunction) {
     ...
     */
    @Nullable V compute(K key, BiFunction<? super K, ? super V, ? extends V> remappingFunction, boolean recordMiss, boolean recordLoad);

    @Override
    @Nullable default V computeIfAbsent(K key, Function<? super K, ? extends V> mappingFunction) {
     ...
     */
    default void invalidateAll(Iterable<?> @Nullable keys) {
        for (Object key : keys) {
            remove(key);
        }
    }
