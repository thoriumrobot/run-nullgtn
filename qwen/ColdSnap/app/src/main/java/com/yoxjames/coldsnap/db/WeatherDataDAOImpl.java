...
public class WeatherDataDAOImpl implements WeatherDataDAO {
    ...
    public Completable saveWeatherData(WeatherData weatherData) {
        return Completable.create(e -> {
            forecastHourRowDAO.addForecastHours(weatherData.getForecastHours().stream().map(forecastHour -> ForecastHourDBMapper.mapToDB(weatherData, forecastHour)).collect(Collectors.toList()));
            e.onComplete();
    ...
    public Observable<WeatherData> getWeatherData(SimpleWeatherLocation weatherLocation) {
        return Observable.fromCallable(() -> forecastHourRowDAO.getForecastHours(weatherLocation.getLat(), weatherLocation.getLon())).filter(list -> !list.isEmpty()).map(forecastHours -> WeatherData.builder().syncInstant(Instant.ofEpochSecond(forecastHours.get(0).getSyncInstant())).forecastHours(forecastHours.stream().map(ForecastHourDBMapper::mapToPOJO).collect(Collectors.toList())).weatherLocation(WeatherLocation.builder().setPlaceString("").setLat(forecastHours.get(0).getLat()).setLon(forecastHours.get(0).getLon()).build()).build()).subscribeOn(Schedulers.io());
    }
