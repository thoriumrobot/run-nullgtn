...
class ColdSnapApplicationModule {
    ...
    static ColdSnapDatabase provideColdSnapDatabase(Context context) {
        return Room.databaseBuilder(context, ColdSnapDatabase.class, "coldsnap-db").fallbackToDestructiveMigration().build();
    }
    ...
    @Provides
    static WeatherService provideWeatherService(WeatherDataDAO weatherDataDAO, HTTPForecastService httpForecastService, WeatherLocationService weatherLocationService, CSPreferences csPreferences) {
    ...
    static OpenWeatherMapHTTPService provideOWMhttpService() {
        ...
        Retrofit retrofit = new Retrofit.Builder().baseUrl("https://api.openweathermap.org/data/2.5/").addCallAdapterFactory(RxJava2CallAdapterFactory.create()).addConverterFactory(GsonConverterFactory.create()).client(httpClient.build()).build();
