...
public class ForecastHourDBMapper {
    ...
    public static ForecastHour mapToPOJO(ForecastHourRow forecastHourRow) {
        return ForecastHour.builder().hour(Instant.ofEpochSecond(forecastHourRow.getHourInstant())).temperature(Temperature.fromKelvin(forecastHourRow.getTempK())).windspeed(Windspeed.fromMetersPerSecond(forecastHourRow.getWindSpeedMps())).uuid(UUID.fromString(forecastHourRow.getUuid())).build();
    }
}
