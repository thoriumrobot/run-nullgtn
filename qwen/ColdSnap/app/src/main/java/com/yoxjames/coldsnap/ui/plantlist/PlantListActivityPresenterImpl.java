...
public class PlantListActivityPresenterImpl extends BaseColdsnapActivityPresenter implements PlantListPresenter {
    ...
    public PlantListActivityPresenterImpl(PlantListMvpView view, PlantService plantService, ImageService imageService, WeatherService weatherService, WeatherLocationService weatherLocationService, PlantListItemReducer plantListItemReducer) {
        super(view);
        this.view = view;
    ...
    public void load() {
        ...
        disposables.add(plantService.getPlants().concatMap(p -> Observable.zip(Observable.just(p), weatherService.getWeatherData(), imageService.getPlantImage(p.getUuid()), plantListItemReducer::reduce)).toList().map(PlantListViewModel::create).observeOn(AndroidSchedulers.mainThread()).subscribe(view::bindView, Throwable::printStackTrace));
        // When clicking on a plant in the list view open the detail view
        disposables.add(view.plantItemClicks().subscribe(view::openPlant));
        disposables.add(view.locationChange().observeOn(Schedulers.io()).doOnNext(loc -> weatherLocationService.saveWeatherLocation(loc).blockingAwait()).observeOn(AndroidSchedulers.mainThread()).subscribe(simpleWeatherLocation -> load(), Throwable::printStackTrace));
        disposables.add(imageService.cleanImagesDirectory().subscribe());
    }
