...
public class PlantListItemReducer {
    ...
    public PlantListItemViewModel reduce(Plant plant, WeatherData weatherData, PlantImage plantImage) {
        return PlantListItemViewModel.builder().plantName(plant.getName()).plantScientificName(plant.getScientificName()).uuid(plant.getUuid()).imageFileName(plantImage.getFileName()).plantStatus(getPlantStatus(plant, weatherData)).build();
    }

    private int getPlantStatus(Plant plant, WeatherData weatherData) {
        @TemperatureComparison
        int temperatureComparison = temperatureComparator.compare(plant.getMinimumTolerance(), ForecastHourUtil.getDailyLow(weatherData.getForecastHours()));
