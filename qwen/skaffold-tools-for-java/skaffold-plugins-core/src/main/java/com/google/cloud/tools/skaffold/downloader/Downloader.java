...
public class Downloader {
    ...
    static long download(URL url, Path destination, long chunkSize) throws IOException {
        ...
        try (FileChannel fileChannel = FileChannel.open(destination, StandardOpenOption.WRITE, StandardOpenOption.CREATE);
            ...
            while (fileChannel.position() < totalSize) {
                fileChannel.position(fileChannel.position() + fileChannel.transferFrom(urlChannel, fileChannel.position(), chunkSize));
            }
            return totalSize;
        }
    }
