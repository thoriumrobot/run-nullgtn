...
final class TrackedReferences {
    ...
    TrackedReferences() {
        leakTrackerMultiMap = Collections.synchronizedMap(new HashMap<Class<?>, Map<LeakType, LeakTracker>>());
    }
    ...
    LeakTracker getOrCreate(Class<?> clazz, LeakType leakType, String className, ActiveTracker.RefGeneratorFactory refGeneratorFactory) {
        if (leakTrackerMultiMap.containsKey(clazz) && leakTrackerMultiMap.get(clazz).containsKey(leakType)) {
            ...
            LeakTracker tracker = LeakTrackerFactory.getTracker(clazz, refGeneratorFactory.createGenerator(), className, leakType);
            addTracker(tracker);
            return tracker;
        }
    }
