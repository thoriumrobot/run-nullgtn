...
public class JavaLayerConfigurations {
    ...
    public enum LayerType {
        ...
        DEPENDENCIES("dependencies"), SNAPSHOT_DEPENDENCIES("snapshot dependencies"), RESOURCES("resources"), CLASSES("classes"), EXTRA_FILES("extra files");
    ...
    public static class Builder {
        ...
        private final Map<LayerType, LayerConfiguration.Builder> layerBuilders = new EnumMap<>(LayerType.class);
         ...
         */
        @Nullable()
        public Builder addFile(LayerType layerType, Path sourceFile, AbsoluteUnixPath pathInContainer, FilePermissions permissions) {
            Preconditions.checkNotNull(layerBuilders.get(layerType)).addEntry(sourceFile, pathInContainer, permissions);
            return this;
        }
        ...
        public Builder addDirectoryContents(LayerType layerType, Path sourceRoot, Predicate<Path> pathFilter, AbsoluteUnixPath basePathInContainer) throws IOException {
            return addDirectoryContents(layerType, sourceRoot, pathFilter, basePathInContainer, ImmutableMap.of());
        }
        ...
        public JavaLayerConfigurations build() {
            ImmutableMap.Builder<LayerType, LayerConfiguration> layerConfigurationsMap = ImmutableMap.builderWithExpectedSize(layerBuilders.size());
            layerBuilders.forEach((type, builder) -> layerConfigurationsMap.put(type, builder.setName(type.getName()).build()));
