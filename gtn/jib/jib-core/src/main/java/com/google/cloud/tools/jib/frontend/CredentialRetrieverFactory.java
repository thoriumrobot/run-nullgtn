...
public class CredentialRetrieverFactory {
    ...
    interface DockerCredentialHelperFactory {
        ...
        DockerCredentialHelper create(@Nullable() String registry, @Nullable() Path credentialHelper);
    }
    ...
    public static CredentialRetrieverFactory forImage(ImageReference imageReference) {
        return new CredentialRetrieverFactory(imageReference, ignored -> {
    ...
    public CredentialRetriever dockerCredentialHelper(Path credentialHelper) {
        return () -> {
            ...
            try {
                ...
                dispatchEvent(LogEvent.info("No credentials for " + imageReference.getRegistry() + " in " + credentialHelper));
    ...
    public CredentialRetriever inferCredentialHelper() {
        ...
        return () -> {
            for (String inferredCredentialHelperSuffix : inferredCredentialHelperSuffixes) {
                try {
                    return Optional.of(retrieveFromDockerCredentialHelper(Paths.get(DockerCredentialHelper.CREDENTIAL_HELPER_PREFIX + inferredCredentialHelperSuffix)));
    ...
    public CredentialRetriever dockerConfig(Path dockerConfigFile) {
        return dockerConfig(new DockerConfigCredentialRetriever(imageReference.getRegistry(), dockerConfigFile));
    }
        ...
        return () -> {
            try {
                ...
                if (dockerConfigCredentials.isPresent()) {
                    dispatchEvent(LogEvent.info("Using credentials from Docker config for " + imageReference.getRegistry()));
    ...
    private Credential retrieveFromDockerCredentialHelper(Path credentialHelper) throws CredentialHelperUnhandledServerUrlException, CredentialHelperNotFoundException, IOException {
        Credential credentials = dockerCredentialHelperFactory.create(imageReference.getRegistry(), credentialHelper).retrieve();
        logGotCredentialsFrom(credentialHelper.getFileName().toString());
        return credentials;
    }

    private void logGotCredentialsFrom(String credentialSource) {
        dispatchEvent(LogEvent.info("Using " + credentialSource + " for " + imageReference.getRegistry()));
    }
