...
public class WeatherServiceImpl implements WeatherService {
    ...
    public WeatherServiceImpl(final WeatherDataDAO dbService, final HTTPForecastService httpForecastService, final WeatherLocationService weatherLocationService, final CSPreferences csPreferences) {
        ...
        weatherDataObservable = weatherLocationService.getWeatherLocation().concatMap(// When nothing is in the cache just use HTTP
        location -> // When nothing is in the cache just use HTTP
        dbService.getWeatherData(location).doOnError(Throwable::printStackTrace).map(weatherData -> weatherData.withWeatherLocation(weatherData.getWeatherLocation().withLocationString(csPreferences.getLocationString()))).onErrorResumeNext(httpForecastService.getForecast(location).doOnNext(this::saveWeatherData)).filter(wd -> !wd.isStale()).mergeWith(httpForecastService.getForecast(location).doOnNext(this::saveWeatherData)).doOnError(Throwable::printStackTrace).firstOrError().toObservable()).compose(ReplayingShare.instance());
    }
