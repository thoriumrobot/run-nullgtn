...
public class PlantDetailTemperaturePickerReducer {
    ...
    public TemperaturePickerViewModel reduce(Plant plant) {
        return TemperaturePickerViewModel.builder().setFormat(getTemperatureFormat()).setMinValue(-100).setMaxValue(100).setValue(temperatureValueAdapter.getValue(plant.getMinimumTolerance())).build();
    }

    @TemperatureFormat
    private int getTemperatureFormat() {
