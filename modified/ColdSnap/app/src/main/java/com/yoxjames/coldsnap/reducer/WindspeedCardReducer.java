...
public class WindspeedCardReducer {
    ...
    public GraphCardViewModel reduce(WeatherData weatherData) {
        final List<Double> windspeeds = weatherData.getForecastHours().stream().map(ForecastHour::getWindspeed).map(Windspeed::getMetersPerSecond).collect(Collectors.toList());
        ...
        final GraphConfig graphConfig = GraphConfig.builder().rangeLowerBound(rangeLowerBound).rangeUpperBound(rangeUpperBound).domainLowerBound(thisMidnightEpochSeconds).domainUpperBound(maxTimeRange).domainStep(86400).rangeStep(1).build();
        return GraphCardViewModel.builder().title("Windspeed").rangeLines(Collections.emptyList()).graphConfig(graphConfig).dataSeries(Collections.singletonList(getWindspeedSeries(weatherData))).rangeLines(Collections.emptyList()).XFormatter(this::localizeDateTime).build();
    }
