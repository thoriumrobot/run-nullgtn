...
public class ImageServiceImpl implements ImageService {
    ...
    public Observable<ActionReply> deleteImagesForPlant(UUID plantUUID) {
        return getPlantImage(plantUUID).flatMap(pi -> imageFileService.deleteImageFile(pi.getFileName(), context).concatWith(plantImageDAO.deletePlantImage(pi.getPlantUUID())));
    }

    @Override
    public Observable<ActionReply> cleanImagesDirectory() {
        return plantImageDAO.getAllPlantImages().flatMap(Observable::fromIterable).map(PlantImage::getFileName).toList().toObservable().flatMap(imageFileService::cleanImageFiles);
    }
}
