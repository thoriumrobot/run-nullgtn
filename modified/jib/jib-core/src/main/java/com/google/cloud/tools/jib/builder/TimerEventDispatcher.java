...
public class TimerEventDispatcher implements Closeable {
    ...
    @VisibleForTesting
    TimerEventDispatcher(EventDispatcher eventDispatcher, String description, Clock clock, @Nullable Timer parentTimer) {
    ...
    private void dispatchTimerEvent(State state, Duration duration, String eventDescription) {
        eventDispatcher.dispatch(new TimerEvent(state, timer, duration, timer.getElapsedTime(), eventDescription));
    }
}
