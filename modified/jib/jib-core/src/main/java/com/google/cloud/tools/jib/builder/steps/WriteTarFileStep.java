...
public class WriteTarFileStep implements AsyncStep<BuildResult>, Callable<BuildResult> {
    ...
    public BuildResult call() throws ExecutionException, InterruptedException {
        ...
        return Futures.whenAllSucceed(dependenciesBuilder.build()).call(this::writeTarFile, listeningExecutorService).get();
    }

    private BuildResult writeTarFile() throws ExecutionException, IOException {
        buildConfiguration.getEventDispatcher().dispatch(LogEvent.lifecycle("Building image to tar file..."));
        ...
        try (OutputStream outputStream = new BufferedOutputStream(FileOperations.newLockingOutputStream(outputPath))) {
            new ImageToTarballTranslator(image).toTarballBlob(buildConfiguration.getTargetImageConfiguration().getImage()).writeTo(outputStream);
        }
