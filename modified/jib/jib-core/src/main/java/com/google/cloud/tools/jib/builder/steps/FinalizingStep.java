...
class FinalizingStep implements AsyncStep<Void>, Callable<Void> {
    ...
    FinalizingStep(ListeningExecutorService listeningExecutorService, BuildConfiguration buildConfiguration, List<AsyncStep<? extends ImmutableList<? extends AsyncStep<?>>>> wrappedDependencyLists, List<? extends AsyncStep<?>> dependencyList) {
        ...
        List<ListenableFuture<?>> dependenciesFutures = new ArrayList<>(wrappedDependencyLists.size() + dependencyList.size());
        ...
        listenableFuture = Futures.whenAllSucceed(dependenciesFutures).call(this, listeningExecutorService);
    }
    ...
    public Void call() throws ExecutionException {
        ...
        for (AsyncStep<? extends ImmutableList<? extends AsyncStep<?>>> wrappedDependency : futureDependencyLists) {
