...
public class ULeak {
    ...
    private static final ConcurrentLinkedQueue<QueuedLeakEvent> PREINITIALIZATION_EVENTS = new ConcurrentLinkedQueue<>();
    ...
    public static synchronized void allowAtMostOne(Object instance, String className) {
        if (trackerSingleton == null) {
            PREINITIALIZATION_EVENTS.add(new QueuedLeakEvent(new WeakReference<>(instance), className, System.currentTimeMillis()));
            return;
        }
        trackerSingleton.registerNewReference(instance, className, System.currentTimeMillis(), SINGLETON);
    }
    ...
    public static synchronized void watchReference(final Object instance, String className) {
        if (trackerSingleton == null) {
            ...
            trackerSingleton.registerNewReference(instance, className, System.currentTimeMillis(), LIFECYCLE);
        }
    }
     ...
     */
    public static synchronized void initialize(final Type type, final ULeakEventReporter reporter, ULeakConfiguration configuration) {
    ...
    static class QueuedLeakEvent {
        ...
        QueuedLeakEvent(WeakReference<Object> weakReference, String className, final long timeEventRegisteredMs) {
