...
public class CSPreferencesPresenter implements MvpPresenter {
    ...
    public void load() {
        ...
        disposables.add(csPreferences.getPreferences().map(preferenceModel -> PreferencesViewModel.builder().setThresholdViewModel(getThresholdVM(preferenceModel)).setFuzzViewModel(getFuzzVM(preferenceModel)).setThresholdSummary(temperatureFormatter.format(preferenceModel.getThreshold())).setFuzzSummary(temperatureFormatter.formatFuzz(preferenceModel.getWeatherDataFuzz())).setTemperatureScale(temperatureFormatter.formatTemperatureScale(preferenceModel.getTemperatureFormat())).setColdAlarmTime(preferenceModel.getColdAlarmTime()).build()).subscribe(view::bindView));
    }
    ...
    public void onEnterThresholdPrefView() {
        disposables.add(view.thresholdChanges().observeOn(Schedulers.io()).map(temperatureValueAdapter::getTemperature).subscribe(csPreferences::setThreshold));
    }

    public void onEnterFuzzprefView() {
        disposables.add(view.fuzzChanges().observeOn(Schedulers.io()).map(this::getKelvinsFromValue).map(Double::floatValue).subscribe(csPreferences::setFuzz));
    }

    private TemperaturePickerViewModel getThresholdVM(PreferenceModel preferenceModel) {
        return TemperaturePickerViewModel.builder().setValue(temperatureValueAdapter.getValue(preferenceModel.getThreshold())).setFormat(csPreferences.getTemperatureFormat()).setMaxValue(100).setMinValue(-100).build();
    }

    private TemperaturePickerViewModel getFuzzVM(@Nullable() PreferenceModel preferenceModel) {
        return TemperaturePickerViewModel.builder().setValue(getValueFromKelvins(csPreferences.getWeatherDataFuzz())).setMinValue(0).setMaxValue(10).setFormat(csPreferences.getTemperatureFormat()).build();
    }
